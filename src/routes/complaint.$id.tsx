  ...
  head: () => ({
    meta: [
      { title: "Complaint record — CivicLens" },
      {
        name: "description",
        content:
          "Public record of a civic complaint in Chennai, with status, timeline and photo evidence.",
      },
      ...
      <main className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6 sm:py-14">
        ...
        {state === "missing" && (
          <div className="rounded-xl border border-dashed border-border bg-card px-6 py-16 text-center">
            ...
            <Link
              to="/"
              className="mt-6 inline-flex items-center gap-1.5 text-sm font-medium text-primary hover:text-accent"
            >
                ...
                <div className="mt-4 flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-xs font-semibold tracking-wider text-muted-foreground uppercase">
                      {c.category}
                      {c.subType ? (
                        <span className="normal-case tracking-normal"> · {c.subType}</span>
                      ) : null}
                    </p>
                    <h1 className="mt-1 font-display text-3xl leading-tight font-bold text-primary sm:text-4xl">
                      {c.title}
                    </h1>
                  </div>
                  <span
                    ...
                    <dd className="mt-1 text-sm">
                      {resolved ? (
                        <span className="inline-flex items-center gap-1.5 font-semibold text-emerald-600">
                          ...
                          {fixedIn !== null
                            ? ` · fixed in ${fixedIn} day${fixedIn === 1 ? "" : "s"}`
                            : ""}
                        </span>
                      ) : (
                        <span
                          className={`inline-flex items-center gap-1.5 font-semibold ${breached ? "text-destructive" : "text-amber-600"}`}
                        >
                          {breached ? (
                          ...
                          {open} day{open === 1 ? "" : "s"}
                          {breached
                            ? ` · exceeded ${SLA_DAYS}-day SLA`
                            : ` · SLA window ${SLA_DAYS} days`}
                        </span>
                      )}
                    </dd>
                  </div>
