...
export const Route = createFileRoute("/")({
  loader: async () => {
    return {
      initialComplaints: await getComplaints(),
    };
  },
  head: () => ({
    meta: [
...
function Index() {
  const { initialComplaints } = Route.useLoaderData();
  ...
  async function submitComplaint(
    data: Omit<Complaint, "id" | "upvotes" | "date" | "status"> & {
      imageUrl?: string | undefined;
      reporterFingerprint?: string | undefined;
      turnstileToken?: string | undefined;
    },
  ): Promise<boolean> {
    const { turnstileToken: _, ...payload } = data;
    const { complaint, error } = await submitComplaintToDb(payload);
        ...
        <div className="mx-auto grid w-full max-w-6xl grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-3 px-4 py-3 sm:px-6">
          ...
          <nav className="hidden items-center gap-1 rounded-full border border-border bg-card p-1 md:flex">
            <Button
              ...
              size="sm"
              variant="ghost"
              className="rounded-full bg-primary/10 px-4 text-primary"
              onClick={goToMap}
            >
              {t("liveMap")}
            </Button>
