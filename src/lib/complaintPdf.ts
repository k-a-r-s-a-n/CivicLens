...
function drawPhotos(ctx: Ctx, slots: PhotoSlot[]) {
    ...
    slots.forEach((s, i) => {
        ...
        if (s.img) {
            ...
            ctx.doc.setTextColor(...MUTED);
            if (s.url) {
                ctx.doc.text("Photo could not be embedded", x + boxW / 2, imgTop + boxH / 2 - 2, { align: "center" });
                ctx.doc.setTextColor(...PRIMARY);
                ctx.doc.setFontSize(7.5);
                ctx.doc.text("Open original photo", x + boxW / 2, imgTop + boxH / 2 + 4, { align: "center" });
...
function footers(doc: jsPDF, generatedLabel: string) {
    ...
    for (let i = 1; i <= n; i++) {
        ...
        doc.text(`Status, dates and counts reflect the CivicLens database at ${generatedLabel}.`, MARGIN, y + 8.5);
...
export async function downloadComplaintPdf(c: Complaint): Promise<string> {
    ...
    let slaColor: Rgb;
    if (resolved) {
        if (fixedIn === null) { slaText = "Resolved (resolution date not recorded)"; slaColor = MUTED; }
        else if (fixedIn <= SLA_DAYS) { slaText = `Met - resolved in ${fixedIn} day${fixedIn === 1 ? "" : "s"} (window ${SLA_DAYS} days)`; slaColor = GREEN; }
        else { slaText = `Missed - took ${fixedIn} days (window ${SLA_DAYS} days)`; slaColor = RED; }
    } else if (breached) {
        slaText = `Breached - open for ${open} days, window is ${SLA_DAYS} days`; slaColor = RED;
    } else {
        slaText = `Within window - ${SLA_DAYS - open} day${SLA_DAYS - open === 1 ? "" : "s"} remaining`; slaColor = AMBER;
    }
    ...
    const rows: Row[] = [
        ...
        { label: "Coordinates", value: `${c.lat.toFixed(6)}, ${c.lng.toFixed(6)}  (open in OpenStreetMap)`, link: osmUrl, color: PRIMARY },
        { label: "Raised on", value: formatDay(c.date) },
        resolved
            ? { label: "Resolved on", value: c.resolvedAt ? formatDay(c.resolvedAt) : "Resolved (date not recorded)" }
            : { label: "Time open", value: `${open} day${open === 1 ? "" : "s"} as of ${formatDay(generated.toISOString())}` },
        ...(resolved ? [{ label: "Time to resolve", value: fixedIn !== null ? `${fixedIn} day${fixedIn === 1 ? "" : "s"}` : "-" }] : []),
        { label: `SLA (${SLA_DAYS} days)`, value: slaText, color: slaColor, bold: true },
        { label: "Community", value: `${c.upvotes} ${c.upvotes === 1 ? "person" : "people"} flagged this issue` },
        { label: "Reported by", value: "Citizen report (anonymous - CivicLens stores no phone numbers or names)" },
    ...
    const slots: PhotoSlot[] = resolved
        ? [
            { label: "Before", caption: "Photo submitted when the complaint was filed.", url: c.imageUrl, img: before },
            { label: "After", caption: "Fix proof uploaded when the complaint was marked resolved.", url: c.fixImageUrl, img: after },
        ]
        : [{ label: "Photo", caption: "Photo submitted when the complaint was filed. An 'after' photo is added on resolution.", url: c.imageUrl, img: before }];
