...
const boundary = boundaryJson as unknown as {
    properties: { name: string; area_km2: number; bbox: { south: number; north: number; west: number; east: number } };
...
function inRing(lng: number, lat: number, ring: Ring): boolean {
    // Ray casting; robust for concave rings
    let inside = false;
...
export function isInsideGCC(lat: number, lng: number): boolean {
    if (lat < south || lat > north || lng < west || lng > east) return false; // cheap reject
    for (const poly of POLYGONS) {
        if (inRing(lng, lat, poly[0]) && !poly.slice(1).some((hole) => inRing(lng, lat, hole))) return true;
    }
    return false;
}
...
export function gccMaskFeature(): GeoJSON.Feature<GeoJSON.Polygon> {
    const world: Ring = [[-180, -90], [180, -90], [180, 90], [-180, 90], [-180, -90]];
    return {
        type: "Feature",
        properties: {},
