 ...
 */
export function useSplash(opts: { minDurationMs?: number; timeoutMs?: number; fadeMs?: number } = {}): SplashState {
    ...
    useEffect(() => {
        ...
        complaints.then(() => { if (!cancelled) { setComplaintsDone(true); targetRef.current += 36; } });
        wards.then(() => { if (!cancelled) { setWardsDone(true); targetRef.current += 36; } });

        // Bar always creeps toward the current target so it never looks frozen
        const creep = setInterval(() => {
            setProgress((p) => (p < targetRef.current ? Math.min(targetRef.current, p + Math.max(0.4, (targetRef.current - p) * 0.08)) : p));
        }, 80);
        ...
        const finish = () => {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => { if (!cancelled) setMounted(false); }, fadeMs);
            }, 350);
        };
    ...
    const stage: Stage = progress >= 100 ? "ready" : !complaintsDone ? "complaints" : !wardsDone ? "wards" : "ready";
    return { mounted, exiting, progress, stage: progress < 12 ? "connecting" : stage, complaintsDone, wardsDone };
}
    ...
    return (
        ...
        >
            ...
            <div
                ...
                style={{ background: "radial-gradient(ellipse at center, color-mix(in oklab, var(--primary) 8%, transparent), transparent 60%)" }}
            />
            ...
            <div className="relative flex w-full max-w-sm flex-col items-center px-6 text-center">
                ...
                <div className="relative flex size-24 items-center justify-center">
                    ...
                    <span className="absolute inset-0 rounded-full border-2 border-primary/30 motion-safe:animate-sonar" style={{ animationDelay: "0.65s" }} />
                    <span className="absolute inset-0 rounded-full border-2 border-primary/20 motion-safe:animate-sonar" style={{ animationDelay: "1.3s" }} />
                ...
                <h1 className="mt-7 font-display text-5xl font-bold tracking-tight text-primary motion-safe:animate-rise" style={{ animationDelay: "0.25s" }}>
                    CivicLens
                </h1>
                <p className="mt-2 text-sm text-muted-foreground motion-safe:animate-rise" style={{ animationDelay: "0.4s" }}>
                ...
                <div className="mt-9 w-full motion-safe:animate-rise" style={{ animationDelay: "0.55s" }}>
                    <div className="h-1.5 w-full overflow-hidden rounded-full bg-muted">
                        <div
                            ...
                            style={{
                                width: `${progress}%`,
                                backgroundImage: progress < 100 ? "linear-gradient(90deg, transparent 0%, rgba(255,255,255,.45) 50%, transparent 100%)" : undefined,
                ...
                <p className="mt-10 text-[10px] tracking-[0.18em] text-muted-foreground/70 uppercase motion-safe:animate-rise" style={{ animationDelay: "0.8s" }}>
...
function Step({ done, label }: { done: boolean; label: string }) {
    return (
        <li className={`flex items-center gap-2 transition-colors ${done ? "text-foreground" : "text-muted-foreground"}`}>
            <span className={`flex size-4 items-center justify-center rounded-full border transition-colors ${done ? "border-emerald-500 bg-emerald-500 text-white" : "border-border"}`}>
                {done ? <Check className="size-2.5" /> : <span className="size-1.5 animate-pulse rounded-full bg-muted-foreground/50" />}
            </span>
            {label}
        </li>
    );
