...
const PAGE_SIZE = 20;
const STATUS_OPTIONS: ("All" | ComplaintStatus)[] = [
  "All",
  "Unresolved",
  "In Progress",
  "Resolved",
  ...
  return (
    <li className="rounded-lg border border-border bg-background p-3.5">
      ...
      <div className="mt-2.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-muted-foreground">
        ...
        <span className="inline-flex items-center gap-1">
          <ThumbsUp className="size-3" /> {c.upvotes} {c.upvotes === 1 ? "person" : "people"}{" "}
          flagged this
        </span>
...
export function WardComplaintLog({ wardId, wardName, refreshToken = 0 }: Props) {
  ...
  }, [statusFilter, categoryFilter, wardId]);

  const counts = useMemo(() => {
    const c = { All: complaints.length, Unresolved: 0, "In Progress": 0, Resolved: 0 } as Record<
      "All" | ComplaintStatus,
      number
    >;
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-[11px] text-muted-foreground">
            {complaints.length} on record · {counts.Unresolved} unresolved · {counts["In Progress"]}{" "}
            in progress · {counts.Resolved} resolved
          </p>
        </div>
        ...
        {isLoading && complaints.length === 0 ? (
          ...
          <>
            <ul className="space-y-2.5">
              {shown.map((c) => (
                <ComplaintEntry key={c.id} c={c} now={now} />
              ))}
            </ul>
