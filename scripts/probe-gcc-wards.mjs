        ...
        for (const url of ENDPOINTS) {
            try {
                ...
                const res = await fetch(url, {
                    method: "POST",
                    headers: { "Content-Type": "application/x-www-form-urlencoded", "User-Agent": "CivicLens/1.0 (ward probe)" },
...
for (const lvl of [10, 9]) {
    ...
    console.log("tag coverage:", Object.entries(tagKeys).sort((a, b) => b[1] - a[1]).slice(0, 10).map(([k, n]) => `${k}(${n})`).join("  "));
    console.table(
    ...
    const refs = list.map((el) => Number(el.tags.ref ?? el.tags["ward:number"] ?? NaN)).filter((n) => !Number.isNaN(n));
    if (refs.length) {
        const have = new Set(refs);
        const missing = Array.from({ length: 200 }, (_, i) => i + 1).filter((n) => !have.has(n));
        console.log(`numeric refs: ${refs.length} · range ${Math.min(...refs)}–${Math.max(...refs)} · missing of 1–200: ${missing.length}${missing.length && missing.length <= 40 ? " → " + missing.join(",") : ""}`);
    }
}