...
const ZONES = [
  ...
  [4, "Tondiarpet", 34, 48],
  [5, "Royapuram", 49, 63],
  [6, "Thiru-Vi-Ka Nagar", 64, 78],
  [7, "Ambattur", 79, 93],
  [8, "Anna Nagar", 94, 108],
  [9, "Teynampet", 109, 126],
  [10, "Kodambakkam", 127, 142],
  [11, "Valasaravakkam", 143, 155],
  [12, "Alandur", 156, 167],
  [13, "Adyar", 168, 182],
  [14, "Perungudi", 183, 191],
  [15, "Sholinganallur", 192, 200],
];
...
for (const el of elements) {
  ...
  if (!m) {
    unnumbered.push({ osm_id: el.id, tags: el.tags });
    continue;
  }
  const n = Number(m[1]);
...
async function lookup(ids, threshold) {
  ...
  url.search = new URLSearchParams({
    ...
    format: "json",
    polygon_geojson: "1",
    polygon_threshold: String(threshold),
  }).toString();
  const res = await fetch(url, { headers: { "User-Agent": UA, Accept: "application/json" } });
...
for (let i = 0; i < allIds.length; i += 50) {
  ...
  for (const r of await lookup(batch, 0.0002))
    if (r.geojson) geomById.set(Number(r.osm_id), r.geojson);
...
const distM = ([x1, y1], [x2, y2]) => {
  const kx = 111320 * Math.cos((y1 * Math.PI) / 180),
    ky = 110570;
...
function assembleRings(ways, label) {
  ...
  while (pool.length) {
    ...
    while (pool.length) {
      ...
      pool.forEach((w, i) => {
        const dHead = distM(last, w[0]),
          dTail = distM(last, w[w.length - 1]);
    ...
    if (key(ring[0]) !== key(ring[ring.length - 1])) {
      ...
      if (g > 0) {
        ...
        BRIDGES.push({
          label,
          d: g,
          a: key(ring[ring.length - 1].map(round)),
          b: key(ring[0].map(round)),
        });
      }
      ring.push(ring[0]);
    }
    if (gaps.length) {
      ...
      console.log(
        `   ${label}: bridged ${gaps.length} gap(s), largest ${Math.max(...gaps).toFixed(0)} m${big.length ? `  ⚠ ${big.length} straight-line bridge(s) > 200 m (OSM data gap, likely missing coastline segment)` : ""}`,
      );
    }
...
if (noGeom.length) {
  console.log(
    `Nominatim had no polygon for wards ${noGeom.map((w) => w.ward).join(",")} — fetching from Overpass (out geom)`,
  );
  ...
  for (const rel of rels) {
    ...
    console.log(
      `   Ward ${wardNo} (relation ${rel.id}): ${wayMembers.length} way members (roles: ${[...new Set(wayMembers.map((m) => m.role || "(empty)"))].join(", ")})`,
    );
    const outers = assembleRings(
      wayMembers.filter((m) => m.role === "outer" || m.role === "").map(toCoords),
      `Ward ${wardNo} outer`,
    );
    const inners = assembleRings(
      wayMembers.filter((m) => m.role === "inner").map(toCoords),
      `Ward ${wardNo} inner`,
    );
  ...
  if (still.length)
    throw new Error(`Still no polygon for wards: ${still.map((w) => w.ward).join(",")}`);

  const big = BRIDGES.filter((b) => b.d > 200);
  for (let i = 0; i < big.length; i++)
    for (let j = i + 1; j < big.length; j++) {
      const same =
        (big[i].a === big[j].a && big[i].b === big[j].b) ||
        (big[i].a === big[j].b && big[i].b === big[j].a);
      console.log(
        `   ${big[i].label} ↔ ${big[j].label}: ${same ? "SAME chord — one shared missing way; the two wards tile each other consistently" : "different chords — independent gaps"}`,
      );
    }
}
...
function roundGeom(g) {
  ...
  if (g.type === "MultiPolygon")
    return { type: "MultiPolygon", coordinates: g.coordinates.map((p) => p.map(rr)) };
...
const zoneGeoms = zones
  .filter((z) => geomById.has(z.osm_id))
  .map((z) => ({ ...z, geom: geomById.get(z.osm_id) }));
...
function zoneShare(geometry) {
  ...
  const dLat = 0.0009,
    dLng = 0.0009 / Math.cos((south * Math.PI) / 180);
  ...
  .map((w) => {
    ...
    if (s && s.zone !== zone) {
      ...
      if (s.share >= 0.85) {
        ...
        resolved.push(
          `Ward ${w.ward}: ${(s.share * 100).toFixed(0)}% of area in "${osmName}" → using Zone ${s.zone} (range table said ${zone})`,
        );
        zone = s.zone;
        zone_name = ZONES.find(([n]) => n === s.zone)[1];
      } else {
        unresolved.push(
          `Ward ${w.ward}: only ${(s.share * 100).toFixed(0)}% in "${osmName}" — keeping range table Zone ${zone}`,
        );
      }
    }
    return {
...
const out = {
  ...
  properties: {
    source: "OpenStreetMap contributors (ODbL) via Overpass + Nominatim",
...
console.log(
  resolved.length
    ? `Zone corrected from OSM (${resolved.length}):\n  ${resolved.join("\n  ")}`
    : "Zone check: all 200 wards agree with the range table",
);
if (unresolved.length)
  console.log(
    `Zone ambiguous, kept range table (${unresolved.length}):\n  ${unresolved.join("\n  ")}`,
  );
