...
function areaKm2(g) {
  ...
  return polys.reduce(
    (s, p) => s + ringAreaKm2(p[0]) - p.slice(1).reduce((h, r) => h + ringAreaKm2(r), 0),
    0,
  );
}
...
console.table(
  candidates.map((c) => ({
    osm_id: c.osm_id,
    name: c.display_name.slice(0, 50),
    km2: c.km2.toFixed(1),
  })),
);

const best = candidates.reduce((a, b) =>
  Math.abs(a.km2 - OFFICIAL_KM2) < Math.abs(b.km2 - OFFICIAL_KM2) ? a : b,
);
if (Math.abs(best.km2 - OFFICIAL_KM2) > 40) {
  throw new Error(
    `Best candidate is ${best.km2.toFixed(0)} km² — not the 426 km² GCC area. Do not write.`,
  );
}
